...
class EvolutionStrategies:
    ...
    def get_flat_params(self):
        # Ensure flat params live on the configured device
        return torch.cat([param.data.view(-1) for param in self.model.parameters()]).to(self.device)

    def set_flat_params(self, flat_params):
        prev_ind = 0
        for param in self.model.parameters():
            flat_size = int(np.prod(list(param.size())))
            # copy to the parameter's device to avoid device-mismatch errors
            param.data.copy_(flat_params[prev_ind:prev_ind + flat_size].view(param.size()).to(param.data.device))
            prev_ind += flat_size
    ...
    def train_step(self, get_reward):
        ...
        # Convert rewards to a tensor on the configured device and handle NaNs/infs
        rewards = torch.tensor(rewards, device=self.device, dtype=torch.float32)
        rewards = torch.nan_to_num(rewards, nan=-1e9, posinf=1e9, neginf=-1e9)
        std = rewards.std()
        if std == 0:
            normalized_rewards = rewards - rewards.mean()
        else:
            normalized_rewards = (rewards - rewards.mean()) / (std + 1e-8)

        # Build gradient on the configured device and make sure noise is on same device
        gradient = torch.zeros_like(self.get_flat_params(), device=self.device)
        for noise, reward in zip(noise_population, normalized_rewards):
            gradient += reward * noise.to(self.device)
